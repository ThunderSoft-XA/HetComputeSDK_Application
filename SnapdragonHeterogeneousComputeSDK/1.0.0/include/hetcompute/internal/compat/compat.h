...
#ifdef _MSC_VER
    #define HETCOMPUTE_STAT _stat
#else
    #define HETCOMPUTE_STAT stat
#endif
...
#else
    void
    hetcompute_exit(int) HETCOMPUTE_GCC_ATTRIBUTE ((noreturn));
#endif // _MSC_VER

void
hetcompute_android_logprintf(int level, const char *format, ...) HETCOMPUTE_GCC_ATTRIBUTE ((format (printf, 2, 3)));
...
/// does not support constexpr. So use a macro to evaluate at runtime.
#ifdef _MSC_VER
    #define hetcompute_constexpr_static_assert(cond, format, ...) \
        do { if (!cond) HETCOMPUTE_FATAL(format, ## __VA_ARGS__); } while (0)
#else
    #define hetcompute_constexpr_static_assert static_assert
#endif
    ...
    static HETCOMPUTE_INLINE
    int pthread_key_create(pthread_key_t *key, void (*destructor)(void*))
    {
        HETCOMPUTE_UNUSED(destructor);
    ...
    #include <string>
        ...
        hetcompute_strerror(int errnum_)
        {
            char buf[1024] = "";
    ...
    static inline size_t hetcompute_getpagesize()
    {
        static size_t _ps = 0;
        if (_ps == 0)
        {
...
#endif
int
hetcompute_string_copy(char* dst, const char* src, int max);
