...
#define HETCOMPUTE_ADD_TO_MEMBUF_MAP(EVENT)                                                                                                \
            template <>                                                                                                                            \
            ...
            protected:
                template <typename EVENT>
                explicit ref_counted_event_handler(EVENT&& e) : _object(e.get_object()), _count(e.get_count())
                {
                }
            ...
            protected:
                template <typename EVENT>
                explicit single_group_event_handler(EVENT&& e) : _group(e.get_group()), _group_id(e.get_group()->get_log_id())
                {
                }
            ...
            protected:
                template <typename EVENT>
                explicit single_task_event_handler(EVENT&& e) : _task(e.get_task()), _task_id(e.get_task()->get_log_id())
                {
                }
            ...
            struct group_canceled_handler : public single_group_event_handler
            {
                ...
                explicit group_canceled_handler(event&& e) : single_group_event_handler(std::forward<event>(e)), _success(e.get_success())
                {
                }
            ...
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(group_canceled);
            ...
            struct group_created_handler : public single_group_event_handler
            {
                ...
                explicit group_created_handler(event&& e)
                    : single_group_event_handler(std::forward<event>(e)), _is_leaf(e.get_group()->is_leaf())
                {
                }
            ...
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(group_created);

            struct group_destroyed_handler : public single_group_event_handler
            {
                typedef events::group_destroyed event;

                explicit group_destroyed_handler(event&& e) : single_group_event_handler(std::forward<event>(e)) {}

                std::string to_string() const;
            };
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(group_destroyed);

            struct group_reffed_handler : public single_group_event_handler
            {
                typedef events::group_reffed event;

                explicit group_reffed_handler(event&& e) : single_group_event_handler(std::forward<event>(e)), _count(e.get_count()) {}
            ...
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(group_reffed);

            struct group_unreffed_handler : public single_group_event_handler
            {
                typedef events::group_unreffed event;

                explicit group_unreffed_handler(event&& e) : single_group_event_handler(std::forward<event>(e)), _count(e.get_count()) {}
            ...
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(group_unreffed);

            struct group_wait_for_ended_handler : public single_group_event_handler
            {
                typedef events::group_wait_for_ended event;

                explicit group_wait_for_ended_handler(event&& e) : single_group_event_handler(std::forward<event>(e)) {}

                std::string to_string() const;
            };
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(group_wait_for_ended);

            struct join_ut_cache_handler : public single_task_event_handler
            {
                typedef events::join_ut_cache event;

                explicit join_ut_cache_handler(event&& e) : single_task_event_handler(std::forward<event>(e)) {}

                std::string to_string() const;
            };
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(join_ut_cache);

            struct object_reffed_handler : public ref_counted_event_handler
            {
                typedef events::object_reffed event;

                explicit object_reffed_handler(event&& e) : ref_counted_event_handler(std::forward<event>(e))
                {
                }
            ...
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(object_reffed);

            struct object_unreffed_handler : public ref_counted_event_handler
            {
                typedef events::object_unreffed event;

                explicit object_unreffed_handler(event&& e) : ref_counted_event_handler(std::forward<event>(e))
                {
                }

                std::string to_string() const;
            };
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(object_unreffed);

            struct task_after_handler : public dual_task_event_handler
            {
                typedef events::task_after event;

                explicit task_after_handler(event&& e) : dual_task_event_handler(std::forward<event>(e)) {}

                std::string to_string() const;
            };
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(task_after);
            ...
            struct task_cleanup_handler : public single_task_event_handler
            {
                ...
                explicit task_cleanup_handler(event&& e)
                    : single_task_event_handler(std::forward<event>(e)), _canceled(e.get_canceled()), _in_utcache(e.get_in_utcache())
                {
                }
            ...
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(task_cleanup);
            ...
            struct task_created_handler : public single_task_event_handler
            {
                ...
                explicit task_created_handler(event&& e)
                    : single_task_event_handler(std::forward<event>(e)), _attrs(e.get_task()->get_attrs())
                {
                }
            ...
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(task_created);

            struct task_destroyed_handler : public single_task_event_handler
            {
                typedef events::task_destroyed event;

                explicit task_destroyed_handler(event&& e) : single_task_event_handler(std::forward<event>(e)) {}

                std::string to_string() const;
            };
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(task_destroyed);

            struct task_finished_handler : public single_task_event_handler
            {
                typedef events::task_finished event;

                explicit task_finished_handler(event&& e) : single_task_event_handler(std::forward<event>(e)) {}

                std::string to_string() const;
            };
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(task_finished);
            ...
            struct task_launched_into_gpu_handler : public single_task_event_handler
            {
                ...
                explicit task_launched_into_gpu_handler(event&& e) : single_task_event_handler(std::forward<event>(e)) {}

                std::string to_string() const;
            };
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(task_launched_into_gpu);
            ...
            struct task_executes_handler : public single_task_event_handler
            {
                ...
                explicit task_executes_handler(event&& e)
                    : single_task_event_handler(std::forward<event>(e)),
                      _group(e.get_task()->get_current_group(hetcompute::mem_order_relaxed)),
                      _group_id(e.get_task()->get_current_group(hetcompute::mem_order_relaxed) ?
                                    e.get_task()->get_current_group(hetcompute::mem_order_relaxed)->get_log_id() :
                                    null_object_id<group>()),
            ...
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(task_executes);

            struct task_sent_to_runtime_handler : public single_task_event_handler
            {
                typedef events::task_sent_to_runtime event;

                explicit task_sent_to_runtime_handler(event&& e) : single_task_event_handler(std::forward<event>(e)) {}

                std::string to_string() const;
            };
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(task_sent_to_runtime);

            struct task_reffed_handler : public single_task_event_handler
            {
                typedef events::task_reffed event;

                explicit task_reffed_handler(event&& e) : single_task_event_handler(std::forward<event>(e)), _count(e.get_count()) {}
            ...
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(task_reffed);

            struct task_unreffed_handler : public single_task_event_handler
            {
                typedef events::task_unreffed event;

                explicit task_unreffed_handler(event&& e) : single_task_event_handler(std::forward<event>(e)), _count(e.get_count()) {}
            ...
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(task_unreffed);
            ...
            struct task_wait_handler : public single_task_event_handler
            {
                ...
                inline explicit task_wait_handler(event&& e)
                    : single_task_event_handler(std::forward<event>(e)), _wait_required(e.get_wait_required())
                {
                }
                std::string to_string() const;
            ...
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(task_wait);

            struct task_wait_inlined_handler : public single_task_event_handler
            {
                typedef events::task_wait_inlined event;

                inline explicit task_wait_inlined_handler(event&& e) : single_task_event_handler(std::forward<event>(e)) {}
                std::string to_string() const;
            };
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(task_wait_inlined);
            ...
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(runtime_disabled);

            struct runtime_enabled_handler : public event_handler_base
            {
                typedef events::runtime_enabled event;

                explicit runtime_enabled_handler(event&& e) : _num_exec_ctx(e.get_num_exec_ctx()) {}
            ...
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(runtime_enabled);

            struct trigger_task_scheduled_handler : public single_task_event_handler
            {
                typedef events::trigger_task_scheduled event;

                explicit trigger_task_scheduled_handler(event&& e) : single_task_event_handler(std::forward<event>(e)) {}

                std::string to_string() const;
            };
            HETCOMPUTE_ADD_TO_MEMBUF_MAP(trigger_task_scheduled);
            ...
            struct unknown_event_handler : public event_handler_base
            {
                ...
                template <typename EVENT>
                explicit unknown_event_handler(EVENT&& e) : _event_id(e.get_id()), _event_name(e.get_name())
                {
                }
            ...
            template <typename Event>
            inline void corelogger::log(Event&& event, event_context& context)
            {
                ...
                static_assert(sizeof(handler) <= buffer_entry::s_payload_size, "Handler is larger than entry payload.");

                entry.reset(count, handler::event::get_id(), context.get_this_thread_id(), s_myloggerid);

                new (entry.get_buffer()) handler(std::forward<Event>(event));
            }

#undef HETCOMPUTE_ADD_TO_MEMBUF_MAP
